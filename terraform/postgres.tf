...
resource "null_resource" "postgres_first_time_install" {
  ...
  provisioner "local-exec" {
    ...
    command = "docker exec --net=ocwa_vnet -v \"$SCRIPT_PATH:/work\" ocwa_postgres psql postgresql://$POSTGRES_USER:$POSTGRES_PASSWORD@ocwa_postgres -f /work/postgres_script.psql"
  }
